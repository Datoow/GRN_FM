...
store = pd.HDFStore('bonemarrow/bone_marrow_cell.h5')
#print(store.keys())
...
pr.columns = cols
#print(pr.columns)
pr.to_csv('data/test data/bone_marrow_Gauss2_%d.tsv' %SK,'\t',index=False)

pr = pd.DataFrame(E2)
pr.columns = cols
#print(pr.columns)
pr.to_csv('data/test data/bone_marrow_Exp_%d.tsv' %SK,'\t',index=False)

kernel_data2 = pd.DataFrame(E3)
kernel_data2.columns = cols
kernel_data2.to_csv('data/test data/bone_marrow_Gauss3_%d.tsv' %SK,'\t',index=False)
...
h = {}
h2 = {}
s = open('bonemarrow/sc_gene_list.txt', 'r')  # gene symbol ID list of sc RNA-seq
for line in s:
...
kernel_data2 = pd.read_csv('data/test data/bone_marrow_Gauss3_%d.tsv' %SK,'\t')
kernel_data = pd.read_csv('data/test data/bone_marrow_Gauss2_%d.tsv' %SK,'\t')
exp_data = pd.read_csv('data/test data/bone_marrow_Exp_%d.tsv' %SK,'\t')
...
f = open('data/test data/bone_marrow_Gene_Ori_Map_%d.tsv' %SK, 'w')
f.truncate()
f.close()
f = open('data/test data/bone_marrow_HSIC_%d.tsv' %SK, 'w')
f.truncate()
f.close()
f = open('data/test data/bone_marrow_PC_%d.tsv' %SK, 'w')
f.truncate()
f.close()
...
for i in range(len(GN_OX)):
  ...
  with open('data/test data/bone_marrow_Gene_Ori_Map_%d.tsv' %SK, 'a') as csvfile:
  ...
  with open('data/test data/bone_marrow_HSIC_%d.tsv' %SK, 'a') as csvfile:
    employee_writer = csv.writer(csvfile, delimiter='\t', quoting=csv.QUOTE_MINIMAL)
    employee_writer.writerow([sum(k_data2[:,0].T*k_data2[:,1].T/1e9)])
    
  with open('data/test data/bone_marrow_PC_%d.tsv' %SK, 'a') as csvfile:
    employee_writer = csv.writer(csvfile, delimiter='\t', quoting=csv.QUOTE_MINIMAL)
...
hsic = pd.read_csv('data/test data/bone_marrow_HSIC_%d.tsv' %SK,'\t',header = None)
...
HSIC.to_csv('data/test data/bone_marrow_HSIC2_%d.tsv' %SK,index=False)

pc = pd.read_csv('data/test data/bone_marrow_PC_%d.tsv' %SK,'\t',header = None)
